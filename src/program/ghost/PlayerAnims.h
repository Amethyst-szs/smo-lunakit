...
namespace PlayerAnims {
    ...
    static constexpr size_t ToValue(Enum type) { return static_cast<std::uint16_t>(type); }

    static constexpr Enum ToEnum(std::uint16_t value) {return static_cast<Enum>(value);}

    static constexpr std::array<const char*, ToValue(Enum::End)> s_Strs {
        // Action Names
        ...
        "WanwanBig",
        // All Cappy Animations
        "CapPointHookStart", "CapPointHookWait", "CapPointWaitMoon", "CapSwitch", "Capture",
        "CaptureWait", "CatchBombCatch", "DemoHackFirstWait", "DemoHackKoopa03", "FlyingStart", "FlyingWait",
        "FlyingWaitCapCatch", "FlyingWaitCapCatchL", "FlyingWaitCapCatchR", "FlyingWaitL", "FlyingWaitR",
        "FlyingWaitRolling", "FlyingWaitSpiral", "LockOn", "NoAction", "PullStay", "Rebound",
        "ReboundEnd", "ReboundStart", "SeparateHipDropLand", "ShibakenCatch", "ShineTowerGlobe", "Stay",
        "StayL", "StayR", "TrampleLockOn", "WaitSeparate"
    };

    static constexpr crc32::HashArray s_Hashes(s_Strs);

    static constexpr Enum FindEnum(std::string_view const& str) {
        return ToEnum(s_Hashes.FindIndex(str));
    }

    static constexpr const char *FindStr(Enum type) {
        const size_t type_ = (size_t)type;
