...
HOOK_DEFINE_TRAMPOLINE(ControlHook) {static void Callback(StageScene * scene){PlayerActorHakoniwa* player = tryGetPlayerActorHakoniwa(scene);
...
HOOK_DEFINE_TRAMPOLINE(NoclipMovementHook) {static void Callback(PlayerActorHakoniwa * player){static bool wasNoclipOn = false;
...
if (isNoclip) {
    ...
    if (al::isPadHoldX(-1) || al::isPadHoldY(-1))
        speedGain += 0.5f;
    if (al::isPadHoldA(-1) || al::isPadHoldB(-1))
        speedGain -= 0.5f;
    if (speedGain <= 0.0f)
        speedGain = 0.0f;
...
HOOK_DEFINE_TRAMPOLINE(SaveHook) {
    static bool Callback(StageScene * scene){if (DevGuiManager::instance()->getSettings()->getStateByName("Autosave")) return Orig(scene);
...
HOOK_DEFINE_TRAMPOLINE(CheckpointWarpHook) {
    static bool Callback(void* thisPtr){if (DevGuiManager::instance()->getSettings()->getStateByName("Always Allow Checkpoints")) return true;
...
HOOK_DEFINE_TRAMPOLINE(GreyShineRefreshHook) {static bool Callback(GameDataHolderWriter writer, ShineInfo const* shineInfo){
    if (DevGuiManager::instance()->getSettings()->getStateByName("Moon Refresh")) return false;
...
HOOK_DEFINE_TRAMPOLINE(DisableMoonLockHook) {
    static int Callback(GameDataHolder * thisPtr, bool* isCrashList, int worldID){int lockSize = Orig(thisPtr, isCrashList, worldID);
...
HOOK_DEFINE_TRAMPOLINE(ButtonMotionRollHook) {
    static bool Callback(void* thisPtr){if (DevGuiManager::instance()->getSettings()->getStateByName("Button Motion Roll")) return true;
...
HOOK_DEFINE_TRAMPOLINE(NoDamageHook){static void Callback(PlayerHitPointData * hitPointData){
    if (!DevGuiManager::instance()->getSettings()->getStateByName("No Damage")) return Orig(hitPointData);
}
}
;
