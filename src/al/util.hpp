...
// getters
    ...
    void addHitSensor(al::LiveActor *actor, al::ActorInitInfo const &initInfo, char const *sensorName, uint typeEnum,
                      float radius, ushort maxCount, sead::Vector3f const &position);
    ...
    bool
    tryGetPlacementId(al::PlacementId *pId, al::PlacementInfo const &placement); //{ return pId->init(placement); };
    ...
    void initActorWithArchiveName(al::LiveActor *actor, al::ActorInitInfo const &initInfo,
                                  sead::SafeString const &archiveName, char const *suffix);
    ...
    void initActorCommon(al::LiveActor *actor, al::ActorInitInfo const &info, char const *dataFolder,
                         char const *archiveName, char const *suffix); // not a real symbol
    void initActorSuffix(al::LiveActor *, al::ActorInitInfo const &, char const *);

    void
    initActorInitInfo(al::ActorInitInfo *, al::Scene const *, al::PlacementInfo const *, al::LayoutInitInfo const *,
                      al::ActorFactory const *, al::SceneMsgCtrl *, GameDataHolderBase *);
    ...
    sead::DrawContext *getSceneDrawContext(
            al::Scene const *); // these two things are all thats needed to setup text writer in the right context
    ...
    bool isExistArchive(sead::SafeString const &filePath);
